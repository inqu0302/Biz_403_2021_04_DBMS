...
-- DECODE(칼럼명, '값', return)
--       칼럼명에 '값' 이 담겨있으면 return 명령을 수행하라
-- 과목명 칼럼에 국어 문자열이 담겨 있으면 해당 레코드의 점수 칼럼값을 
-- 표시하고 그렇지 않으면 (null)값을 표시
SELECT 학번,
...
-- 제2정규화 되어 있는 Table의 테이블을 PIVOT하여 보고서 형태의 table로 변환

-- 위의 SQL을 학번으로 GROUPING 하고
-- 각 점수를 합산(SUM())하면 DBMS의 SQL에서는 (NULL) + 숫자 = 0 + 숫자와 같다
