...
/*
iolist 테이블과 Prod 테이블간에 상품으로 Join하여 Null 값이 없는것이 확인되었다.

1. iolist 테이블에 상품코드 칼럼을 추가하고
2. prod 테이블에서 상품코드를 가져와 iolist 테이블에 저장
3. iolist 테이블과 prod 테이블간의 상품코드를 기준으로 Join을 할 수 있도록
        테이블 변경을 시작
...
-- 생성된 io_pcode 칼럼에 io_pname 칼럼의 상품이름에 해당하는 코드 데이터를 
-- tbl_product에서 가져와서 저장을 해야한다.

-- 테이블의 데이터를 변경하기 위한 DML
-- tbl_iolist 전체를 반복하면서 io_pcode 칼럼의 값을 갱신
-- 이때 tbl_iolist의 상품으로 tbl_product 데이터를 조회하여 일치하는 데이터가
--  있으면 그 중에 상품코드 칼럼의 값을 가져와서 io_pcode 칼럼에 저장
...
/*
iolist 전체데이터를 보여달라
iolist 데이터의 상품이름을 product 테이블에서 조회하여 일치하는 상품이 있으면
리스트를 보일때 같이 보여달라 라는 Sub Query
*/
SELECT io_pname IO,
(
...
-- tbl_iolist의 상품코드 칼럼에 저장된 값과 tbl_product의 상품코드를 JOIN하여
-- 데이터 조회
...
거래처 코드를 만들고 Table을 작성한 다음 데이터 import 

iolist 에 io_dcode칼럼을 추가하고 데이터를 UPDATE 수행
*/

-- 1. iolist 로부터 거래처명, 대표자명 칼럼을 기준으로 중복되지 않은
--      데이터를 조회 거래처명,대표자명 순으로 정렬
...
-- import 된 거래처 정보와 매입 매출 정보를 join하여 null값이 있는지 확인
-- tbl_iolist에 io_dcode 칼럼추가, CHAR(5)
ALTER TABLE tbl_iolist
ADD io_dcode CHAR(5);

-- 거래처정보 테이블에서 거래처 코드를 조회하여 tbl_iolist의 io_dcode칼럼에 update
UPDATE tbl_iolist IO
SET IO.io_dcode = (
...
SELECT io_date, io_time, io_pcode,
        p_name, io_dcode, d_name,
        d_ceo, DECODE(io_inout, '1', '매입', '2', '매출') AS 구분,
        io_qty, io_price
FROM tbl_iolist
