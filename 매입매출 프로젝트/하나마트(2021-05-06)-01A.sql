...
-- 매입할때 매입부가세 발생
-- 팬매할때 매출부가세 발생
-- 매출부가세 -- 매입부가세를 계산하여 일년에 2 ~ 4회 부가가치세를 납부

-- 농사를 지어서 쌀 20KG을 생산하여 판매를하면
-- 5만원 정도의 금액에 판매
-- 쌀을 공장에서 가공하여 생산품(공산품)으로 만들게 되면
-- 실제 20KG 의 쌀을 직접 판매한것보다 더 비싼 가격에 판매하게 된다
-- 이때 실제 쌀보다 더 많은 이익이 발생하게 되므로
-- 가치가 부가 되었다 라고 표현한다
-- 가치가 부가 된 만큼 세금을 납부하도록 한다
-- 부가 가치 세(Value Add Tax, VAT)
-- 매입을 할때는 매입금액의 10% 만큼으로 세금을 포함하여 매입하고
-- 매출을 할때는 매출금액의 10% 만큼 세금으로 포함하여 판매한다

-- 매입매출 데이터에서 보면 매입금액은 부가세 10%를 제외한 금액으로 입력하고
-- 매출금액은 부가세 10%가 포함된금액으로 입력한다.
-- 샘플데이터의 매입금액은 VAT제외금액이고 매출금액은 VAT포함 금액이다
-- 매입과 매출 데이터로 지난 1년간 납부한 VAT금액을 계산
...
-- 2020-01-01 부터 2020-06-30 기간동안 거래된 리스트를 거래처 별로 조회
SELECT io_dname,
    SUM(DECODE(io_inout, '매입', io_total, 0)) AS 매입합계,
    SUM(DECODE(io_inout, '매출', io_total, 0)) AS 매출합계
FROM tbl_iolist
-- WHERE io_date >= '01/01/2020' AND io_date <= '30/06/2020'
WHERE io_date BETWEEN '01/01/2020' AND '30/06/2020'
GROUP BY io_dname
ORDER BY io_dname;

-- GROUP BY 는 같은 데이터를 묶어서 출력
...
4. 매입단가가 0인 데이터는 매출데이터에서 임의로 생성하기
    매출 단가의 80%를 매입단가로 하고, 부가세를 제외한 금액으로 계산
    E2항목의 값을 0.8로 곱하면 E2의 80% 가격이 되고
    다시 그 금액을 1.1로 나누면 부가세를 제외한 가격이 된다.
    =ROUND(IF(C2 = 0, ((E2 * 0.8)/1.1),C2),0)
    
5. 매출단가가 0 인 데이터는 매입데이터에서 임의로 생성하기
...
테이블을 생성하여 데이터를 import했다

매입매출 데이터와, 상품정보, 거래처 정보를 Join 하기 위해서는 매입매출 데이터에
상품코드, 거래처 코드가 있어야 한다.
그러나 현재 데이터는 코드 칼럼이 없이 이름 칼럼만 있는 상태이다

매입매출 데이터에 상품코드, 거래처코드 칼럼을 추가하고 새 table을 Join 할수 
있도록 변경하기
*/
