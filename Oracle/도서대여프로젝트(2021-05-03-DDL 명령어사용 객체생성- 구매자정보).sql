...
사용자의 권한에대한
GRANT : 권한부여, REVOKE : 권한삭제, DENY : 사용자차단

DDL(Data Dafinition Language)정의어
DB 및 TABLE 에 대한 정의
CREATE : 생성, DROP : 삭제, ALTER : 변경(재정의)

DML(Data manipulation Languager) 조작어
TABLE의 내용에 대한 조작
SELCET : 조회, DELEATE : 삭제, UPDATE : 수정, INSERT : 삽입

CRUD (기본 데이터 처리기능)
INSERT(CREATE), SELECT(READE), UPDATE, DELETE
*/


        
-- tbl__book_rent 와 tbl_books, tbl_buyer table의 참조무결성 설정
-- 대상 Table은 다종관계 Table 
-- tbl_buyer 데이터 1개(1명) 고객이 다수의 tbl_book_rent Table에 포함이 된다
-- tbl_buyer 1 : tbl_book_rent N = 1 : 다 의 관계이다.
-- N의 Table에서 FK 설정을하고, 1의 Table과 관계를 맺는다.
...
다른 table의 칼럼을 참조하고 있지만 모든 칼럼이 중복 값을 가질수 있는 관계로
단일 칼럼으로 PK를 설정할 수 없다.

PK를 성정하고, 더불어 주문관련 리스트를 보는데 사용할 br_seq칼럼을 만들고 
이 칼럼으로 PK를 설정했다.

book_rent 테이블에 데이터를 INSERT 하려고 할때 항상 유일한 값으로 
PK를 설정해야하는 어려움이 있다.
보통 이러한 칼럼은 일련번호 순으로 만드는게 오라클이외의 다른 DBMS에는 
일련번호를 자동으로 만들어주는 Table 속성이 있다.
오라클(11버전 이하)에서는 그러한 기능이 없다.

mySQL의 경우 칼럼 속성에 AUTO_INCREMENT로 설정을 하면 그 칼럼은 INSERT할때
별도 값을 지정하지 않아도 항상 유일한 일련번호를 자동생성한다.

오라클에는 일련번호를 생성하기 위한 별도의 객체가 존재한다.
*/
-- 오라클에서는 일련번호를 생성하는 SEQUENCE객체
-- DDL명렁어를 사용하여 SEQ 객체 생성
-- 이름을 명명할때 SEQ_로 시작하고 뒤에 적용할 대상 table 명을 
-- 붙여서 생성한다.
...
VALUES(seq_book_rent.NEXTVAL, '2021-05-03', '9791188850501', 'B0001');

SELECT  * FROM tbl_book_rent;

-- 이미 만들어져 있는 SEQ삭제
DROP SEQUENCE seq_boo_rent;

--기존에 사용하던 SEQ를 삭제하고 다시 생성할 경우 반드시 적용회는 table의 칼럼
--값을 확인해야 한다.
SELECT MAX(br_seq), FROM tbl_book_rent;

-- SEQ START 값을 적용하는 테이블의 SEQ 최대값보가 크게 설정하자
CREATE SEQUENCE seq_book_rent
START WITH 11
INCREMETN BY 1;

INSERT INTO btl_book_rent(br_seq, br_sdate, br_inbs, br_bcode)
VALUES(seq_bookr_rent.NEXTVAL, '2021-04-20', '9791186805398', 'B0020');
...
-- FK 관계가 설정되지 않은 TABLE간에서 조회되는 데이터가 실제와 다를수 있다.
SELECT 
    BR.br_sdate AS 대여일,
    BR.br_bcode AS 회원코드,
    BU.bu_name AS 회원명,
...
-- LEFT (OUTER) JOIN
-- 테이블간 FK가 설정되지 않고 참조하는 테이블의 데이터가 마련되지 않은경우
