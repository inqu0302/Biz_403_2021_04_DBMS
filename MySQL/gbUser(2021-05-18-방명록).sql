...
values(
'2021-05-18', '10:32:00', 'inqu0302', 'inqu0302@naver.com', '12345', '내일은 쉬는날');
...
-- 날짜와 시간기준으로 최근글이 제일 먼저보이도록
SELECT * FROM tbl_guest_book
ORDER BY gb_date DESC, gb_time DESC;

-- UPDATE, DELETE를 수행할때는 2개 이상의 레코드에 영향을 미치는 명령은
-- 매우 신중하게 실행해야 한다.
-- 가장 좋은 방법은 변경, 삭제하고자 하는 데이터가 여러개 있더라도 가급적
-- PK를 기준으로 1개씩 처리하는것이 가장좋다
...
SELECT 
	IF(MOD(FLOOR(RNAD() * 100), 2)=0, '짝수', '홀수');
